...
async function getSpecificProfile(ctx, id) {
  ...
  try {
    ...
    const handle = profile.handle;
    const metadata = profile.metadata;
    const pictureLink = (profile.picture && !profile.picture.original.url.startsWith('https://statics-polygon-lens'))
...
/*
async function searchProfile(ctx, handle) {
  const chatId = ctx.chat.id;
  const query = `
  query Profile {
...
async function displaySearchProfile(conversation, ctx) {
  const chatId = ctx.chat.id;

  await ctx.reply("What is the handle you are searching for?? 🤗");
  const handle = await conversation.form.text();

  try {
    const [profileExist, message, metadata, pictureLink] = await searchProfile(ctx, handle);
    ...
    if (
      ...
      const [message, metadata] = await getDefaultProfile(ctx, address);
      const menu = new InlineKeyboard()
        //.url("View MetaData", `${metadata}`)
        .text("Back to my Profile", `see_default_profile:${walletLink[chatId]}`)
        ;
...
async function seePosts(ctx, id) {
  ...
  try {
    ...
    await ctx.reply(`Posts Found: ${posts}`);
...
async function explorePosts(ctx, option) {
  ...
  try {
    ...
    console.log("Number:" + posts);
    await ctx.reply(`Suggesting ${posts} posts`);
    ...
    if (response && response.data.explorePublications.items.length > 0) {
      for (let i = 0; i < response.data.explorePublications.items.length; i++) {
        try {
          ...
          if (i === response.data.explorePublications.items.length - 1) {
            await ctx.reply(`😱 An error occurred with post number ${i+1}. Post could not be rendered. Use /start.`);}
            else{
              await ctx.reply(`😱 An error occurred with post number ${i+1}. Continuing with the next post...`);
            }
        }
      }
    }
...
async function searchPublication(conversation, ctx) {
  ...
  try {
    ...
    console.log(response.data.search.items)
    await ctx.reply(`${number_post} Posts found matching search for '${searched_post}'`);
